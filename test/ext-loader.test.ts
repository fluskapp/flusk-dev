...
describe("loadExtensions", () => {
	...
	beforeEach(async () => {
		repo = await setupTestHome("ah-ext-loader-");
	});
	...
	it("records the flows and events an extension registers, and wires the handlers", async () => {
		await writeExtension(
			...
			`export default (ah) => {
				ah.flow({ name: "ship" })
				ah.flow({})
				ah.on("run:end", (e) => ah.log("saw " + e.reason))
			}`,
		);
		const lines: string[] = [];
	...
	it("drops the partial registrations of a setup that throws halfway", async () => {
		...
		await writeExtension(
			...
			"halfway.js",
			`export default (ah) => {
				ah.tool({ name: "early", description: "d", parameters: { type: "object" }, mode: "parallel", execute: async () => ({ output: "" }) })
	...
	it("reports a syntactically broken file as a failure rather than crashing", async () => {
		await writeExtension("global", repo, "broken.js", "export default (ah => {");
