/**
 * `ah flow run` isolates itself, exactly as `ah run` does.
 *
...
withLang("ah flow run isolates itself, as ah run does", () => {
	...
	it("runs on its own branch, and says which one and what the gate will be", async () => {
		...
		expect(text()).toMatch(/isolation: branch ah\//);
		expect(text()).toContain("verify: true");
		expect(git("rev-parse", "--abbrev-ref", "HEAD").trim()).toMatch(/^ah\//);
	});
