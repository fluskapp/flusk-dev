/**
 * `ah flow run` for real, offline: LangChain's own FakeListChatModel behind the
 * runtime's `chat` seam, a temp AH_HOME, a temp repo whose gate is `true` (or
 ...
 * still must exit 1 — and the isolation `ah run` already insists on, since a
...
async function gate(commands: string[]): Promise<void> {
	await writeFile(join(repo, ".ah.json"), JSON.stringify({ verify: commands }));
}

beforeEach(async () => {
	repo = await setupTestHome("ah-flow-run-");
