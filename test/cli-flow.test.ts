...
beforeEach(async () => {
	repo = await setupTestHome("ah-cli-flow-");
...
it("lists a user flow from .ah/flows and names a broken one", async () => {
	await mkdir(join(repo, ".ah", "flows"), { recursive: true });
	const tidy =
	...
	await writeFile(join(repo, ".ah", "flows", "tidy.json"), tidy);
	await writeFile(join(repo, ".ah", "flows", "broken.json"), JSON.stringify({ nodes: [] }));
...
it("says how to install the runtime when the optional packages are absent", async () => {
	...
	expect(text).toContain("ah flow needs the LangGraph runtime:");
