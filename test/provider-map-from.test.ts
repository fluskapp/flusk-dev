...
describe("fromPiMessage", () => {
	...
	it("maps every pi stopReason to a hit stopReason", () => {
		...
		for (const [pi, hit] of table) expect(fromPiStopReason(pi)).toBe(hit);
	});

	it("maps pi usage to hit usage via cost.total", () => {
