...
export interface GraphFixture {
	...
	root: string;
	/** An AH_HOME of its own, so one test's graph file cannot leak into another. */
	home: string;
...
export function graphProject(): GraphFixture {
	const fx = make(mkdtempSync(join(tmpdir(), "ah-graph-")));
...
export function cloneFixture(fx: GraphFixture): GraphFixture {
	const copy = make(mkdtempSync(join(tmpdir(), "ah-graph-")));
...
/** An ah session card: same shape, but its ref is a session key, not a sha. */
