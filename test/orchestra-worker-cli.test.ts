...
beforeAll(() => {
	bin = mkdtempSync(join(tmpdir(), "ah-orch-bin-"));
	repo = realpathSync(mkdtempSync(join(tmpdir(), "ah-orch-repo-")));
	initRepo(repo);
	...
	cfg = cfgWith([
		...
		{ id: "absent", kind: "cli", command: "ah-no-such-agent-4b21", args: [] },
...
it("reports availability from the backend without spawning the agent", async () => {
	...
	expect(await worker.available(spec("gone", "absent"))).toMatchObject({
		ok: false,
		reason: "ah-no-such-agent-4b21 not found on PATH",
	});
