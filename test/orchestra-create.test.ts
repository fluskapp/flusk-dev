...
beforeEach(() => {
	...
	const tmp = realpathSync(mkdtempSync(join(tmpdir(), "flusk-create-")));
	home = join(tmp, "home");
	repo = join(tmp, "repo");
...
describe("agentForTask", () => {
	it("creates a spec offline, writes markdown, and finds it again on reload", async () => {
		...
		expect(made.path).toBe(join(repo, ".flusk", "agents", `${made.spec.name}.md`));
	...
	it("takes a model's draft but keeps the name a safe filename", async () => {
		...
		expect(existsSync(join(repo, ".flusk", "agents", "etc-pwned-agent.md"))).toBe(true);
	});
