...
beforeEach(async () => {
	...
	home = await mkdtemp(join(tmpdir(), "flusk-home-"));
	repoRoot = await mkdtemp(join(tmpdir(), "flusk-repo-"));
...
describe("Session", () => {
	...
	it("stores files under FLUSK_HOME/sessions/<slug> with a stable slug", async () => {
		const s = Session.create({ task: "t", repoRoot, model });
		s.close();
		const dir = sessionsDir(repoRoot);
		expect(fluskHome()).toBe(home);
		...
		const otherRepo = await mkdtemp(join(tmpdir(), "flusk-Other Repo!-"));
		try {
