...
import type { DocReply, OutlineReply } from "../src/ui/api-doc.js";
...
beforeAll(async () => {
	home = mkdtempSync(join(tmpdir(), "ah-apidoc-home-"));
	work = mkdtempSync(join(tmpdir(), "ah-apidoc-work-"));
	outside = mkdtempSync(join(tmpdir(), "ah-apidoc-out-"));
