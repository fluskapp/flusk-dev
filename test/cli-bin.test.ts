...
/** Executes the built dist binary in a child process with an isolated FLUSK_HOME. */
function runBin(args: string[]): Promise<BinResult> {
	return new Promise((resolve, reject) => {
		const child = spawn(process.execPath, [bin, ...args], {
			env: { ...process.env, FLUSK_HOME: join(tmp, "home") },
		});
		let stdout = "";
		let stderr = "";
...
beforeAll(async () => {
	tmp = await mkdtemp(join(tmpdir(), "flusk-cli-bin-"));
	repo = join(tmp, "repo");
