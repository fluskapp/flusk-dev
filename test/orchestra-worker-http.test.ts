...
beforeAll(async () => {
	repo = realpathSync(await setupTestHome("ah-orch-http-"));
	initRepo(repo);
	...
	cfg = cfgWith([
		...
		{ id: "shell", kind: "cli", command: "ah-no-such-cli-91aa", args: [] },
	]);
});
...
it("drives ah's own tool loop over the endpoint and observes what it changed", async () => {
