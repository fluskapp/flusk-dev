...
describe("extension scopes", () => {
	...
	beforeEach(async () => {
		repo = await setupTestHome("ah-ext-scope-");
	});
	...
	it("never lets a repo vouch for itself through its own files", async () => {
		await mkdir(join(repo, ".ah"), { recursive: true });
		await writeFile(join(repo, ".ah.json"), JSON.stringify({ trusted: true, extensions: true }));
		await writeFile(join(repo, ".ah", "trusted-projects.json"), JSON.stringify([repo]));
		await writeExtension("project", repo, "theirs.js", toolExtension("theirs", "theirs"));
	...
	it("ignores a malformed trust file rather than trusting everything", async () => {
		...
		await writeFile(join(ahHome(), "trusted-projects.json"), "{ not json");
		await writeExtension("project", repo, "theirs.js", toolExtension("theirs", "theirs"));
