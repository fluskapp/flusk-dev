/**
 * The cache round trip on its own: written under the ah home, read back
 ...
 * the ah home rather than anything the URL itself chose.
 */
...
beforeAll(() => {
	home = mkdtempSync(join(tmpdir(), "ah-webcache-"));
...
it("writes under the ah home, under a name the URL did not choose", () => {
	...
	expect(path.startsWith(realpathSync(ahHome()))).toBe(true);
