...
beforeEach(async () => {
	repo = await setupTestHome("ah-lang-planner-");
});
...
it("loads user flows from the project and from ah's home, project first", async () => {
	const node = { id: "a", kind: "summarize", about: "say it" };
	await write(join(repo, ".ah", "flows"), "mine.json", { name: "mine", entry: "a", nodes: [node] });
	await write(join(ahHome(), "flows"), "shared.json", {
		name: "shared",
		entry: "a",
		nodes: [node],
	});
