/**
 * What `flusk watch` does before it polls anything: honour a request to leave no
...
beforeEach(async () => {
	repo = await setupTestHome("flusk-watch-cmd-");
}, SLOW);
...
it("flusk goal refuses too, because its task graph IS the store", async () => {
	await writeHomeConfig({ memory: { enabled: false } });
	const cap = capture();
...
it("reclaims expired cooldowns from the ledger on the way in", async () => {
	...
	const log = await readFile(nsPath(storeDir(), FLUSK_NS), "utf8");
