...
import type { Answerer, AskContext } from "../src/ui/ask-types.js";
...
beforeAll(async () => {
	home = mkdtempSync(join(tmpdir(), "ah-ask-home-"));
	work = mkdtempSync(join(tmpdir(), "ah-ask-work-"));
	...
	mkdirSync(join(app, ".ah", "agents"), { recursive: true });
	writeFileSync(
		join(app, ".ah", "agents", "ghost-runner.md"),
