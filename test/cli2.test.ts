...
beforeEach(async () => {
	repo = await setupTestHome("hit-cli2-");
}, SLOW);
...
test("--dry on the real path prints model/kind/tools/isolation and system prompt without any provider call", async () => {
	...
	try {
		...
		expect(text).toContain("You are hit, an autonomous coding agent.");
...
test("--dry honors --kind and --model overrides and shows the isolation plan for git repos", async () => {
	...
	expect(cap.text()).toMatch(/isolation: branch hit\/<run-id>/);
}, SLOW);
...
test("fake run with isolation lands checkpoint commits on a hit/<id> branch and prints the review hint", async () => {
	await initGitRepo(repo);
	...
	expect(branch).toMatch(/^hit\/[0-9a-f-]{8}$/);
	expect(git(repo, "log", "--format=%s")).toContain("hit: turn 1");
	expect(git(repo, "show", "HEAD:out.txt")).toBe("data\n");
	expect(cap.text()).toMatch(/1 commit on hit\//);
