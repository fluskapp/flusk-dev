...
import { creditFor, improveFromRun } from "../src/features/flows/improve.js";
...
beforeEach(async () => {
	repo = await setupTestHome("flusk-lang-scores-");
});
...
it("promotes a fix from a run runFlow can actually produce", async () => {
	...
	expect((await improveFromRun(real, { repoRoot: repo, store: mem.store })).lessons).toBe(1);
	expect((await mem.rows(LESSONS_NS))[0]?.confidence).toBe(0.9);
});
...
it("refuses to promote when the last verification did not pass", async () => {
	...
	expect((await improveFromRun(unverified, { repoRoot: repo, store: mem.store })).lessons).toBe(
		0,
	);
	expect(await mem.rows(LESSONS_NS)).toHaveLength(0);
});
