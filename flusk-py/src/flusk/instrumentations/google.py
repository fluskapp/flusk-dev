# --- BEGIN GENERATED ---
"""Google AI (Gemini) SDK instrumentation — gen_ai.* OTel spans."""
...
TRACER_NAME = "flusk-google"
_GOOGLE_PATCHED = False


def patch_google() -> None:
    """Monkey-patch Google AI (Gemini) SDK to emit OTel spans."""
    global _GOOGLE_PATCHED
    if _GOOGLE_PATCHED:
        return
    try:
        mod = importlib.import_module("google_generative_ai")
    except ImportError:
        return
    _patch_method(mod, "generateContent")
    _GOOGLE_PATCHED = True
...
def _patch_method(mod: Any, method_path: str) -> None:
    ...
    def patched(self: Any, *a: Any, **kw: Any) -> Any:
        ...
        with tracer.start_as_current_span(
            f"google {model}", kind=SpanKind.CLIENT,
        ) as span:
