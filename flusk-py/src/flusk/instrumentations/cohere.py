# --- BEGIN GENERATED ---
"""Cohere SDK instrumentation — gen_ai.* OTel spans."""
...
TRACER_NAME = "flusk-cohere"
_COHERE_PATCHED = False


def patch_cohere() -> None:
    """Monkey-patch Cohere SDK to emit OTel spans."""
    global _COHERE_PATCHED
    if _COHERE_PATCHED:
        return
    try:
        mod = importlib.import_module("cohere_ai")
    except ImportError:
        return
    _patch_method(mod, "chat")
    _COHERE_PATCHED = True
...
def _patch_method(mod: Any, method_path: str) -> None:
    ...
    def patched(self: Any, *a: Any, **kw: Any) -> Any:
        ...
        with tracer.start_as_current_span(
            f"cohere {model}", kind=SpanKind.CLIENT,
        ) as span:
