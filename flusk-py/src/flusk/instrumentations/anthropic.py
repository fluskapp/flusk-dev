# --- BEGIN GENERATED ---
"""Anthropic SDK instrumentation — gen_ai.* OTel spans."""
...
TRACER_NAME = "flusk-anthropic"
_ANTHROPIC_PATCHED = False


def patch_anthropic() -> None:
    """Monkey-patch Anthropic SDK to emit OTel spans."""
    global _ANTHROPIC_PATCHED
    if _ANTHROPIC_PATCHED:
        return
    try:
        mod = importlib.import_module("anthropic_ai_sdk")
    except ImportError:
        return
    _patch_method(mod, "messages.create")
...
def _patch_method(mod: Any, method_path: str) -> None:
    ...
    def patched(self: Any, *a: Any, **kw: Any) -> Any:
        ...
        with tracer.start_as_current_span(
            f"anthropic {model}", kind=SpanKind.CLIENT,
        ) as span:
