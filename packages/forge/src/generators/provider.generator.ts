...
export async function generateProvider(
  ...
  if (nameOrYamlPath.endsWith('.yaml') || nameOrYamlPath.endsWith('.yml')) {
    ...
    await generateAll(cfg, root, result);
  } else {
...
async function generateAll(
  cfg: ProviderYaml, root: string, result: ProviderResult
): Promise<void> {
  await writeProviderFile(root, result, `packages/business-logic/src/llm-call/providers/${cfg.name}.pricing.ts`, renderPricing(cfg));
  await writeProviderFile(root, result, `packages/business-logic/src/llm-call/providers/${cfg.name}.span-config.ts`, renderSpanConfig(cfg));
  await writeProviderFile(root, result, `packages/business-logic/src/llm-call/providers/${cfg.name}.test.ts`, renderProviderTest(cfg));
  await writeProviderFile(root, result, `packages/otel/src/instrumentations/${cfg.name}.ts`, renderInstrumentation(cfg));
  await writeProviderFile(root, result, `packages/otel/src/instrumentations/${cfg.name}.test.ts`, renderInstrumentationTest(cfg));
  await patchCreateSdk(cfg, root, result);
  await patchParseSpan(cfg, root, result);
}

async function patchCreateSdk(
  cfg: ProviderYaml, root: string, result: ProviderResult
): Promise<void> {
...
async function writeProviderFile(
  root: string, result: ProviderResult, relPath: string, content: string
): Promise<void> {
...
async function generateLegacy(
  name: string, root: string, result: ProviderResult
): Promise<void> {
