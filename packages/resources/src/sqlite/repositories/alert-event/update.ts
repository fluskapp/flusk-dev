...
/**
 * Update a Alert event
 */
export function update(
  db: DatabaseSync,
  id: string,
  data: Partial<Omit<AlertEventEntity, 'id' | 'createdAt' | 'updatedAt'>>,
): AlertEventEntity | null {
  const sets: string[] = [];
  ...
  const stmt = db.prepare(
    `UPDATE alert_events SET ${sets.join(', ')} WHERE id = ? RETURNING *`,
  );
