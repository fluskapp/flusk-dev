...
export class EventBusClient {
    ...
    streamPrefix;
    running;
    constructor(redis, streamPrefix = 'flusk:events') {
        this.redis = redis;
    ...
    async publish(eventType, payload) {
        ...
        const messageId = await this.redis.xadd(streamKey, '*', // Auto-generate ID
        'eventType', eventType, 'payload', JSON.stringify(payload), 'timestamp', timestamp.toString());
     ...
     */
    async subscribe(eventType, options, handler) {
        const streamKey = `${this.streamPrefix}:${eventType}`;
        const { groupName, consumerName, blockMs = 5000, count = 10 } = options;
        // Ensure consumer group exists
        try {
            await this.redis.xgroup('CREATE', streamKey, groupName, '0', // Start from beginning
        ...
        while (this.running.get(subscriptionKey)) {
            try {
                ...
                const results = (await this.redis.call('XREADGROUP', 'GROUP', groupName, consumerName, 'BLOCK', blockMs.toString(), 'COUNT', count.toString(), 'STREAMS', streamKey, '>' // Only new messages
                ));
                ...
                for (const [, messages] of results) {
                    for (const [messageId, fields] of messages) {
                        try {
                            ...
                            console.error(`Error handling message ${messageId}:`, handlerError);
     ...
     */
    stopSubscription(eventType, groupName, consumerName) {
        const subscriptionKey = `${eventType}:${groupName}:${consumerName}`;
