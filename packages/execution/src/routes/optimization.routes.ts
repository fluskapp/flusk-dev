...
export async function optimizationRoutes(
  ...
  fastify.post('/', {
    ...
    const created = OptimizationRepository.createOptimization(fastify.db, request.body as Record<string, unknown>);
  ...
  fastify.put('/:id', {
    ...
    const { id } = request.params as { id: string };
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- request body validated by schema
    const updated = OptimizationRepository.updateOptimization(fastify.db, id, request.body as Record<string, unknown>);
