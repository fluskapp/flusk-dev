...
// --- BEGIN CUSTOM ---
const CreateOptimizationSchema = Type.Omit(OptimizationEntitySchema as unknown as TObject, ['id', 'createdAt', 'updatedAt']);
...
export async function optimizationRoutes(
  ...
  fastify.post('/', {
    ...
    const created = OptimizationRepository.createOptimization(fastify.db, request.body as Static<typeof CreateOptimizationSchema>);
  ...
  fastify.put('/:id', {
    ...
    const { id } = request.params as { id: string };
    const updated = OptimizationRepository.updateOptimization(fastify.db, id, request.body as Static<typeof CreateOptimizationSchema>);
