...
// --- BEGIN CUSTOM ---
const CreateAnalyzeSessionSchema = Type.Omit(AnalyzeSessionEntitySchema as unknown as TObject, ['id', 'createdAt', 'updatedAt']);
...
export async function analyzeSessionRoutes(
  ...
  fastify.post('/', {
    ...
    const created = AnalyzeSessionRepository.create(fastify.db, request.body as Static<typeof CreateAnalyzeSessionSchema>);
  ...
  fastify.put('/:id', {
    ...
    const { id } = request.params as { id: string };
    const updated = AnalyzeSessionRepository.update(fastify.db, id, request.body as Static<typeof CreateAnalyzeSessionSchema>);
