...
<p align="center">
  <strong>Open-source LLM cost intelligence for Node.js and Python teams.</strong><br />
...
<p align="center">
  ...
  <a href="https://pypi.org/project/flusk/"><img src="https://img.shields.io/pypi/v/flusk?style=for-the-badge&color=3775A9" alt="PyPI"></a>
...
That's it. No accounts, no API keys, no config files. Flusk intercepts every LLM call via [OpenTelemetry](https://opentelemetry.io/), calculates real costs, and prints a report with savings opportunities. Same CLI, same analysis — whether you're writing TypeScript or Python.
...
- **Per-model breakdown** — see exactly what each model costs you (OpenAI, Anthropic, Google Gemini, AWS Bedrock)
...
**Zero intrusion.** No wrappers, no monkey-patching, no SDK to import. Flusk hooks into the runtime via OpenTelemetry auto-instrumentation and captures LLM calls at the HTTP layer. Your code stays unchanged — in both Node.js and Python.
...
#### Flusk Analyze Action (built-in)

Use the official GitHub Action to get automatic cost comments on every PR:

```yaml
- uses: adirbenyossef/flusk-dev/.github/actions/analyze@main
  with:
    script: './src/index.ts'
    explain: true
    budget: 100
...
|-------|-------------|----------|---------|
| `script` | Path to the script to analyze | ✅ | — |
| `provider` | LLM provider (openai, anthropic, google, local) | | `openai` |
| `budget` | Maximum allowed cost in USD | | — |
| `explain` | Enable cost explanation insights | | `false` |
| `explain-provider` | Provider for generating explanations | | `openai` |
| `token` | Authentication token for the LLM provider | | — |

| Output | Description |
|--------|-------------|
| `total-cost` | Total cost of all LLM calls in USD |
| `total-calls` | Total number of LLM API calls |
| `insights-json` | JSON array of cost optimization insights |

<!-- TODO: Add screenshot of PR cost comment -->

## Python Support

Flusk now supports **both Node.js and Python** from the same YAML schema definitions. The Python package is generated by the forge system — same entities, same SQLite schema, cross-language compatible.
...
- Same YAML schemas generate both TypeScript and Python code
- Same CLI commands and UX across both languages
- Same SQLite schema — analyze a Node.js app, view the report from Python (or vice versa)
- `flusk recipe python-package` regenerates all Python code from YAML
- 10 npm packages + 1 PyPI package, all from the same source of truth

See the [Python package README](./flusk-py/README.md) and [Dual-Language Architecture](./docs/generators/dual-language.md) for details.
...
90% of the Flusk codebase is **generated from YAML schema files**. Entities, types, repositories, routes, migrations, and tests — all defined in YAML and produced by our generator framework. The same YAML schemas now generate both **TypeScript** (Node.js) and **Python** code.
...
| [Python Guide](./docs/generators/python-guide.md) | Using Flusk with Python |
| [Dual-Language Architecture](./docs/generators/dual-language.md) | How YAML → TS + Python works |
