 ...
 *
 * The path is inside the ah home and is proved so by the existing path jail in
...
import { ahHome, repoSlug } from "../session/paths.js";
import { decode, encode, type Triple } from "./triples.js";

export function graphDir(): string {
	return join(ahHome(), "graph");
}

/** One file per project, named by the collision-safe slug the rest of ah uses. */
...
/**
 * Proves the target is under the ah home before anything opens it (S5). The
...
function within(name: string): string {
	...
	return resolveWithin([ahHome()], join(dir, name), dir);
}
