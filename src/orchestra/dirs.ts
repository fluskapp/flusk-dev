 ...
 * under `<repo>/.ah/agents/` has to be found by the very next reload, and a
 ...
 * a symlink dropped in a repo's agents directory cannot make ah read a file
 * from outside the repo (or outside the ah home) and label it project-scoped.
 */
...
/** `<repo>/.ah/agents` — where dynamically created specs are written. */
export function projectAgentsDir(repoRoot: string): string {
	return join(repoRoot, ".ah", "agents");
}

/**
 * Scan order: the user's own agents, then ah's own generated ones, then the
 ...
 */
export function agentDirs(repoRoot: string, home: string = ahHome()): AgentDir[] {
	return [
