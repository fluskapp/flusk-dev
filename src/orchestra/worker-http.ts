/**
 * The "http" worker: an OpenAI-compatible endpoint driving flusk's own loop.
 *
 ...
 * OpenRouter — and because the loop is flusk's, that model gets the real tools,
...
/**
 * These endpoints do not advertise a window and flusk only needs it to decide
...
export function httpWorker(cfg: FluskConfig, ctx: LoopCtx): Worker {
	return {
		kind: "http",
...
function resolve(cfg: FluskConfig, spec: AgentSpec): Resolved {
	if (spec.worker !== "http") {
