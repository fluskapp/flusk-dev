 ...
 * to delegate: `task`, which spawns another ah loop on ah's own provider.
 ...
 * and the work may land in ah's own loop, in an installed agent CLI working
 * in the directory, or in an OpenAI-compatible model driving ah's tools.
 *
...
export function createDelegateTool(ctx: LoopCtx, cfg: AhConfig, repoRoot: string): Tool<typeof params> {
	return {
		name: "delegate",
		description:
			"Delegate a self-contained task to a named coding agent, or let ah choose one. " +
			"Unlike `task`, the work may run in an installed agent CLI or another model, " +
			"not only in ah's own loop. Omit `agent` to route by the task's description. " +
