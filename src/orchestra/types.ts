/**
 * The orchestrator's frozen contract: how ah delegates ONE task to ONE coding
 * agent that may not be ah.
 *
 * Today delegation is src/tools/task.ts -> runSubagent, which can only build
 * another ah loop on ah's own provider. That is one worker out of three: this
 * machine already has `claude`, `codex` and `kimi` installed and
 * authenticated, and any OpenAI-compatible endpoint can drive ah's own tool
 ...
 *   builtin    shipped inside ah
 *   global     <ah home>/agents/*.md            the user's own
 *   project    <repo>/.ah/agents/*.md           authored by whatever was cloned
 *   generated  <ah home>/agents/generated/*.md  written by ah itself
 *
 ...
 * spec may load while `<repo>/.ah/extensions/*.js` (arbitrary code) may not:
...
export type AgentWorkerKind =
	/** ah's own loop (createAgent), the runSubagent path generalised. */
	| "internal"
	/** An installed agent CLI, spawned as a subprocess in a directory. */
	| "cli"
	/** An OpenAI-compatible endpoint driving ah's own tool loop. */
	| "http";
...
export interface WorkerTask {
	 ...
	 * src/safety/paths.ts. A worker writes here or under the ah home, nowhere
