 ...
 */
import type { MemoryClient, MemVerdict } from "../memory/client-types.js";
import { lessonFact } from "../memory/facts.js";
import { promoteLessons } from "../watch/promote.js";
import type { FlowResult, FlowStep } from "./types.js";

type Fix = { failed: FlowStep; fixed: FlowStep };

/** Extracted, not yet confirmed — abagraph parks this as a Candidate, and
 * promotion is exactly the act of confirming one (docs/vocabulary.md). */
const EXTRACTED = 0.7;
...
export async function writeLessons(
	...
	for (const fix of found) {
		...
		await client.transact(ns, [{ ...base, source }]);
	}
	const verdict: MemVerdict = verified(r) ? "ALLOW" : "WARN";
	return (await promoteLessons(client, ns, at.slug, at.runId, verdict)).promoted;
}
