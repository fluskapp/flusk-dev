/**
 * Where a flow run is written down — in the two places ah already looks.
 * Facts go through the memory client as ordinary `Run:` rows (outcome, stage,
 * failed_because — docs/vocabulary.md): a flow run IS a run, so it needs no new
...
import { runFact, vocabularyFact } from "../memory/facts.js";
...
export interface FlowRecordCfg {
	...
	/** Defaults to the repo namespace (src/memory/namespaces.ts). */
	ns?: string;
...
function factsFor(r: FlowResult, runId: string): MemFactInput[] {
	const out: MemFactInput[] = [runFact.outcome(runId, r.outcome)];
	for (const s of r.state.steps) {
		out.push(vocabularyFact(`Run:${runId}`, "stage", `${s.nodeId}:${s.ok ? "done" : "failed"}`));
		if (!s.ok) out.push(runFact.failedBecause(runId, clean(`${s.nodeId}: ${s.note ?? "failed"}`)));
	}
	return out;
}
	...
	if (cfg.client) {
		...
		try {
			for (const f of factsFor(result, runId)) {
				...
				seen.add(key);
				// One assert per transact: abagraph rejects two asserts on the same
				// (subject, predicate) in one call, even for coexist rows.
				await cfg.client.transact(ns, [f]);
				facts++;
			}
		} catch (err) {
			notes.push(`memory not updated: ${errText(err)}`);
		}
	}
