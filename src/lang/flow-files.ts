/**
 * User flows on disk: `<project>/.ah/flows/*.json` and `<ahHome>/flows/*.json`.
 *
...
export async function loadFlows(projectRoot: string): Promise<LoadedFlows> {
	const local = await readDir(join(projectRoot, ".ah", "flows"));
	const home = await readDir(join(ahHome(), "flows"));
	const flows: FlowSpec[] = [];
