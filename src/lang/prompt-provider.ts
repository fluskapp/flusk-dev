 ...
 * `ah prompt` already takes. A second composer would be a second set of ranking
...
export interface NodePrompt {
	text: string;
	tokens: number;
	/** Every section's origin, in order — the audit trail `ah prompt` prints. */
	sources: string[];
 ...
 * `renderBlocks`, so what a node is sent and what `ah prompt` prints cannot
 ...
 * same trade `ah prompt` makes. A flow run in an unindexed checkout should
