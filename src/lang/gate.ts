 ...
 * the report claim-check `ah run` uses, because a flow that grades its own
 ...
 *
 * That second one only means anything because `ah flow run` starts from a clean
...
import { formatEvidence, runVerify } from "../verify/gate.js";
import { checkReportText } from "../verify/report-check.js";
import type { NodeOutcome } from "./types.js";

export const NO_GATE = "no verification command detected — set verify in .ah.json";
...
export function gate(report: string, base: NodeOutcome, cfg: AhConfig, at: GateAt): NodeOutcome {
