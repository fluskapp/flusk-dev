/**
 * ah's model routing, spoken as a LangChain chat model.
 *
 * ah decides WHICH model a step gets — `chooseModel` over the learned scores
 ...
 */
import type { ChatBackendConfig, ModelChoice } from "../config/types.js";
import {
	type Importer,
	LANGCHAIN,
	LANGCHAIN_ANTHROPIC,
...
/**
 * A chat model for an already-routed ah model choice: @langchain/anthropic
...
export async function chatModelFor(
	...
	if (backend === undefined || (backend.baseUrl ?? "") === "") {
		return no(
			`no chat backend for ${choice.provider}/${choice.id} — set chat.backends[].baseUrl in ~/.ah/config.json`,
		);
	}
