...
#doc-grip { left: -3px; top: 0; bottom: 0; width: 7px; cursor: col-resize; }
...
.dw-name { font: 600 13px var(--font-code); color: var(--text); }
.dw-kind { font-size: 11px; color: var(--dim); }
/* Which engine answered — the reason a thin panel is thin. */
.dw-badge {
	font: 10px var(--font-code); color: var(--dim); padding: 0 4px;
...
#doc-body { flex: 1; overflow: auto; min-height: 0; padding-bottom: 24px; }
.dw-sec { padding: 6px 10px 8px; border-bottom: 1px solid var(--border); }
.dw-sec h4 {
	margin: 0 0 4px; font-size: 10.5px; font-weight: 600; color: var(--dim);
	text-transform: uppercase; letter-spacing: .5px;
}
.dw-sub {
	margin: 6px 0 2px; font-size: 11px; font-weight: 600; color: var(--dim);
}
.dw-sig pre, .dw-sig pre.code {
	margin: 0; padding: 6px 8px; background: var(--code-bg); border: 1px solid var(--border);
	white-space: pre-wrap; overflow-x: auto;
}
.dw-doc { font-size: 12.5px; }
.dw-doc p, .dw-doc ul, .dw-doc pre { margin: 4px 0; }

/* @param / @returns: a definition table, not a paragraph of tag soup. */
.dw-tags { display: grid; grid-template-columns: auto minmax(0, 1fr); gap: 2px 8px; }
.dw-k { font: 11.5px var(--font-code); color: var(--accent); }
.dw-v { min-width: 0; }
...
.dw-row {
	...
	padding: 0 4px; cursor: pointer; border-left: 2px solid transparent;
}
...
.dw-row .glyph {
	flex: none;
	font: 10px var(--font-code); color: var(--dim); border: 1px solid var(--border); padding: 0 3px;
}
.dw-t {
...
.dw-why {
	flex: none; max-width: 48%; min-width: 0; font-size: 11px; color: var(--dim);
	overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
}
.dw-at, .dw-loc { flex: none; margin-left: auto; font: 11px var(--font-code); color: var(--dim); }
/* The act row is a sentence, not a fact, so it is the one row allowed to wrap. */
.dw-act { height: auto; padding: 3px 4px; white-space: normal; }
.dw-count { font-size: 11.5px; color: var(--text); }
.dw-act { color: var(--accent); font-size: 11.5px; }
.dw-note { margin-top: 5px; font-size: 11px; color: var(--dim); }
.dw-none { font-size: 11.5px; color: var(--dim); }
.dw-empty { padding: 14px 12px; font-size: 12px; color: var(--dim); }
`;
