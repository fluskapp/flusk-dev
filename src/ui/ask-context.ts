...
import { indexedFile } from "./api-doc-util.js";
...
export async function assembleContext(cfg: AhConfig, req: ContextRequest): Promise<AskContext> {
	const file = await indexedFile(cfg, req.file);
	if (file === null) {
		return empty([
			"Nothing is open that ah has indexed. Open a source file — the project tree, " +
