/**
 * The `ah` object handed to an extension's setup function, and the ledger of
 * what that extension actually did with it.
 *
 * WHY THIS FILE EXISTS: the contract promises ah can TELL the user what is
...
import type { AhEvent, EventBus } from "../core/events.js";
import type { Tool } from "../tools/tool.js";
import type { AhApi } from "./types.js";

export interface ApiSink {
	config: AhConfig;
	home: string;
	repoRoot: string;
	/** Absent when only inspecting (`ah doctor`): handlers are recorded, not attached. */
	events?: EventBus;
...
function assertTool(tool: Tool): void {
	if (typeof tool?.name !== "string" || tool.name.length === 0) {
		throw new Error("ah.tool() needs a non-empty name");
	}
	if (typeof tool.execute !== "function") {
		throw new Error(`ah.tool("${tool.name}") needs an execute function`);
	}
	if (typeof tool.parameters !== "object" || tool.parameters === null) {
		throw new Error(`ah.tool("${tool.name}") needs a parameters schema`);
	}
}
...
export function createExtensionApi(
	name: string,
	sink: ApiSink,
): { api: AhApi; record: ExtensionRecord } {
