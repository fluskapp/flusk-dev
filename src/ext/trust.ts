 ...
 *
 * WHY THIS FILE EXISTS: `<repo>/.ah/extensions/*.js` is arbitrary JavaScript
 ...
 * exactly this line for config layers — `~/.ah/…` is the user's own file and is
 * trusted, `<repo>/.ah.json` is authored by whoever wrote the repo and is not —
 * and the same line has to hold here, harder: a config layer is data, an
 * extension is code.
 *
 * So trust lives ONLY in the user layer: `<ah home>/trusted-projects.json`, a
...
export function trustFilePath(): string {
	return join(ahHome(), "trusted-projects.json");
}
...
/** The sentence ah shows instead of silently loading nothing. */
