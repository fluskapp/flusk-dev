 ...
 *
 *  1. Module kind. A `.js` file under ~/.flusk has no package.json above it, so
 ...
 *     process (`flusk ui`, watch mode) an edited extension would keep serving the
...
export async function importSetup(source: string): Promise<ExtensionSetup> {
	...
	if (typeof setup !== "function") {
		throw new Error(
			"an extension must default-export a setup function: export default (flusk) => { … }",
		);
	}
