/**
 * The extension loader: the file that finally makes flusk hackable.
 *
 * WHY THIS FILE EXISTS: src/ext/types.ts has described the seam for a while and
 * nothing ever loaded anything. This walks `~/.flusk/extensions/*.js` then
 * `<repo>/.flusk/extensions/*.js`, runs each setup, and hands back what they
 * registered.
 *
 ...
 *    invisible refusal is how "flusk ignores my extension" becomes an hour of
...
import { type Candidate, discoverExtensions } from "./discover.repository.js";
import { importSetup } from "./import-setup.repository.js";
import { isProjectTrusted, untrustedReason } from "./trust.repository.js";
...
export async function loadExtensions(opts: LoadExtensionsOptions): Promise<ExtensionLoad> {
	...
	for (const candidate of await discoverExtensions(opts.repoRoot)) {
		...
		if (candidate.scope === "project" && !projectTrusted) {
			// Recorded, not a failure: nothing went wrong, flusk refused on purpose.
			// It lands in `extensions` with a reason so `flusk doctor` can say so.
