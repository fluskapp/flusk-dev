...
export function cliWorker(cfg: FluskConfig): Worker {
	return {
		kind: "cli",
...
function probe(cfg: FluskConfig, spec: AgentSpec): WorkerAvailability {
	if (spec.worker !== "cli") {
...
async function runCli(cfg: FluskConfig, task: WorkerTask): Promise<WorkerResult> {
