/**
 * Pure mapping between hit's provider-agnostic types and pi-ai wire types.
...
export function toPiContext(system: string, msgs: Msg[], tools: ToolSchemaJson[]): Context {
	...
	const piTools: PiTool[] = tools.map((t) => ({
		...
		// Typebox schemas are TSchema at runtime; hit carries them as plain JSON.
...
/** Events with no hit equivalent (start/…_start/…_end bookkeeping) map to null. */
