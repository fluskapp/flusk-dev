 ...
 * any of it — the same rule `ah run` applies (run-cmd.ts).
 *
 ...
 * times. `ah run` refuses a non-git repo by default for exactly that reason;
 * `ah flow run` used to do neither the refusal nor the branch.
 *
 ...
 * line the user has to act on — when the repo is not one ah is willing to run
 * unattended in.
 */
export function startFlowIsolation(repo: string, cfg: AhConfig, opts: IsolationOpts): string {
	if (opts.off === true) return "isolation: off (--no-isolation)";
	if (!isGitRepo(repo)) {
		if (cfg.isolation.requireGit) {
			throw new Error(
				`${repo} is not a git repository; ah isolates flow runs on a branch (pass --no-isolation to override)`,
			);
		}
...
export function verifyNotice(repo: string, repoConfig?: RepoConfig): string {
	...
	return cmds.length === 0
		? "verify: no command detected — set verify in .ah.json"
		: `verify: ${cmds.join(" && ")}`;
}
