/**
 * `ah flow` — the whole flow runtime, in three subcommands and no prompt.
 *
 *   ah flow run <task>   the shape is chosen from the task unless --flow names
 *                        one; --dry composes and prints every node's prompt
 *                        without calling a model, so a run can be audited
 *                        before it is paid for.
 *   ah flow list         built-in and user flows as one arrow chain each.
 *   ah flow resume <id>  continue a run from its last checkpoint.
 *
...
export const FLOW_USAGE = `Usage:
  ah flow run <task> [--flow <name>] [--project <p>] [--max-cost <usd>] [--dry] [--json]
                     [--no-isolation] [--allow-dirty]
  ah flow list [--json]
  ah flow resume <runId>
`;
...
export async function flowCmd(opts: FlowCmdOpts): Promise<CliOutcome> {
	...
	if (opts.flow !== undefined && flowByName(opts.flow, flows) === null) {
		write(out, `ah: no flow named "${opts.flow}" — try: ${flows.map((f) => f.name).join(", ")}\n`);
		return "error";
	}
	...
	try {
		...
		write(out, `ah: ${e instanceof Error ? e.message : String(e)}\n`);
		return "error";
	}
	...
	if ((await loadLang(opts.lang)) === null) {
		write(out, `ah flow needs the LangGraph runtime:\n  ${langMissing()}\n`);
		return "error";
	}
