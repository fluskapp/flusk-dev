/**
 * Flag validation for `ah flow`, kept out of main.ts so the dispatcher stays a
 * readable table of commands — the same split `ah run` uses (run-args.ts).
 *
 ...
 * `ah run` (run-args.ts): a flow's verify nodes execute the repo's own gate
...
export function parseFlowArgs(positionals: string[], v: Values, cwd: string): ParsedFlowArgs {
	...
	if (sub === undefined || !SUBS.has(sub)) {
		return { ok: false, error: "ah: flow takes run, list or resume\n" };
	}
	if (sub !== "list" && (arg === undefined || arg.trim() === "")) {
		const what = sub === "run" ? "<task>" : "<runId>";
		return { ok: false, error: `ah: flow ${sub} needs ${what}\n` };
	}
