...
export async function queryCommand(
	...
	if (command === "runs") {
		...
		if (!Number.isInteger(limit) || limit <= 0) return fail("flusk: -n must be a positive integer\n");
		runsCmd({ limit });
		return true;
	}
	if (command === "search") {
	...
	if (command === "context") {
		if (arg === undefined) return fail(USAGE);
		// Scoped to the current repo like `flusk run`: the context a run gets is
		// built against the repo it is about to edit, so `flusk context` asked from
	...
	if (command === "prompt") {
		if (arg === undefined) return fail(USAGE);
		// Scoped to the current repo like `flusk run`, `flusk goal` and `flusk watch`;
