 ...
 * returned tools correctly, and nothing in src/ ever called it: `ah run` handed
 * `createAgent` the hardcoded DEFAULT_TOOLS, so a working
 * `~/.ah/extensions/*.js` registered a tool the agent could never call and said
 ...
 * refusal (S1: `<repo>/.ah/extensions/*.js` is arbitrary code shipped by a repo
...
export async function toolbelt(opts: ExtToolsOpts): Promise<Tool[]> {
	...
	for (const ext of load.extensions) {
		if (ext.error !== undefined) write(opts, `ah: extension ${ext.name} skipped — ${ext.error}\n`);
	}
