/**
 * What `flusk flow` prints: the library as one arrow chain per flow, and the dry
...
/** `flusk flow list` — built-in and user flows, each as `plan -> code -> verify`. */
export function renderFlowList(lib: FlowLibrary): string {
	...
	lines.push(
		...(lib.user.length === 0
			? ["  (none) — drop a flow JSON in .flusk/flows/ to add one"]
			: flowLines(lib.user)),
	);
...
/**
 * `flusk flow run <task> --dry`. The header says where the shape came from, then
...
export function renderDryPlan(plan: DryPlan): string {
	...
	const lines = [
		...
		// The same disclosure `flusk prompt` makes: evidence from other projects is
